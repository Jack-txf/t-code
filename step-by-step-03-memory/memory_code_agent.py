...
from memory.memory_persistence import load_memory, build_system_prompt_with_memory, DEFAULT_MEMORY_PATH, \
    execute_save_memory_tool, SAVE_MEMORY_TOOL_SCHEMA
# 1.导入环境变量
...
async def main_async():
    print("=========== t-code MEMORY模式启动 ==========")
    init_llm()

    # ── 第二节读取持久记忆，注入 system prompt ─────────────────────────────────────────────────────
    ...
    # save_memory 作为内置工具追加到 schema 列表
    tool_schemas = tool_schemas + [SAVE_MEMORY_TOOL_SCHEMA]
